...
export function SiteNav({ active, compact = false }: { active: 'home' | 'chronicle' | 'contact' | 'entry'; compact?: boolean }) {
  return (
        ...
        {ITEMS.map((item) => {
          ...
          return (
            <Link
              ...
              className={`rounded-full px-2.5 py-1.5 font-mono text-[10px] transition-all sm:px-3 sm:text-[11px] ${
                selected ? 'bg-ink text-base shadow-sm' : 'text-muted hover:bg-raised hover:text-ink'
              }`}
            >
              {item.label}
            </Link>
