        ...
        <div className="mt-12">
          {beats.map((beat, i) => (
            <Reveal key={beat.id} delay={i < 4 ? i * 40 : 0}>
              <Link
                href={beat.href}
                className="group relative block overflow-hidden border-b border-line/60 py-5 transition-colors hover:bg-surface/25 sm:py-7"
              >
                  ...
                  <span className="flex items-center justify-start gap-3 sm:justify-end">
                    {beat.cover && (
                      ...
                      <img
                        ...
                        className="hidden h-14 w-24 shrink-0 rounded border border-line/60 object-cover opacity-80 transition duration-500 group-hover:opacity-100 group-hover:scale-[1.03] sm:block"
                      />
                    )}
                    <span
                      aria-hidden
